...
public:
    ...
    //kernel parameters
    double *d_SP, *d_SPVM, *d_SPM, *d_SPR, *d_SPRC, *d_SPRT, *d_SPRP,*d_SPEX,*d_SPREX,*d_QELC;
    int *d_ISPR,*d_ISPRC, *d_TREACL,*d_TREACG,*d_ISPRK,*d_NSPEX,*d_ISPEX,*d_NEX,*d_NSLEV;
...
__device__ int index(int a,int b,int size2){
    long long m =a*size2 +b;
    return m;
}

__device__ int index(int a, int b, int c, int size2, int size3){
    long long m = (b+a*size2)*size3 + c;
    return m;
}

__device__ int index(int a, int b, int c, int d, int size2, int size3, int size4){
    long long m = ((a*size2+b)*size3+c)*size4 + d;
    return m;
}
...
__device__ void SROT(curandState* globalState, int &L,double &TEMP,double &ROTE, GAS* gas, CALC *calc)
{
    ...
    double A,B,ERM;
    //
    if(gas->d_ISPR[index(1,L,(gas->MSP+1))] == 2){
...
__device__ void SVIB(curandState* globalState, int &L,double &TEMP,int &IVIB, int &K, GAS *gas, CALC *calc)
{
    ...
    // CALL RANDOM_NUMBER(RANF)
    calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
    N=-logf(calc->RANF)*TEMP/gas->d_SPVM[index(1,K,L,(gas->MMVM+1),(gas->MSP+1))];                 //eqn(4.10)
...
void ALLOCATE_GAS()
{
    ...
    gas->d_allocate(9,gas->MSP+1,gas->MSP+1,gas->SPM);
            ...
            if(gas->ISPRC[L][M] > 0){
                ...
                for(I=1;I<=2;I++){
                    ...
                    for(J=0;J<=JMAX;J++){
                        ...
                        file_9<< "CDF level dissoc "<<J<<" "<<gas->SPRP[I][L][M][J]<<endl;
                    }
                }
            }
        }
                    ...
                    if(gas->SPEX[3][K][L][M] > 0.e00){         //exothermic (reverse) exchange reaction
                        ...
                        for(I=1;I<=2;I++){
                            for(J=0;J<=JMAX;J++){
                                ...
                                file_9<<"Cumulative "<<I<<" "<<J<<" "<<gas->SPREX[I][K][L][M][J]<<endl;
                            }
                        }
                    }
                }
...
void OUTPUT_RESULTS()
{
    ...
    if(calc->ICLASS==0 && calc->ISF==1){
        ...
        file_10<<std::right<<setw(15)<<calc->TOTCOL<<setw(15)<<A<<setw(15)<<output->VAR[11][1]<<setw(15)<<output->VAR[8][1]<<setw(15)<<output->VAR[9][1]<<setw(15)<<output->VAR[10][1]<<setw(15)<<output->VAR[22][1]<<endl;
        //file_10<<std::right<<setw(15)<<A<<setw(15)<<output->VAR[8][1]<<setw(15)<<output->VAR[9][1]<<setw(15)<<output->VAR[8][1]-output->VAR[9][1]<<endl;
...
__global__ void cuda_collisons(curandState* globalState, MOLECS *molecs, OUTPUT *output, GEOM_1D *geom, GAS *gas, CALC *calc)
{
    ...
    //OUTPUT output;
    //GEOM_1D geom;
    int NN,M,MM,L,LL,K,KK,KT,J,I,II,III,NSP,MAXLEV,IV,NSEL,KV,LS,MS,KS,JS,IIII,LZ,KL,IREC,NLOOP,IA,IDISS,IEX,NEL,NAS,NPS,
    ...
    // double *VRC = &buffer[0];
    // double *VCM = &buffer[3];
    // double *VRCP = &buffer[6];
    // double *VRCT =&buffer[9];
    //*A=&buffer[13];
    //*AA=&buffer[14];
    double VRC[4],VCM[4],VRCP[4];
    //   //N,M,K working integer
    ...
    //cout<<"START COLLISIONS"<<endl;
        ...
        // if(N==54634){
        //     printf("start\n %e %e\n %e %e\n %e %e\n ",geom->XB[1],geom->XB[2], geom->d_CCELL[5][666], geom->d_CCELL[5][666666],geom->d_CCELL[3][666], geom->d_CCELL[3][666666]);
        //     printf("%d %d\n %d %d\n %d %d\n",geom->d_ICCELL[2][666], geom->d_ICCELL[2][666666],geom->d_ICCELL[3][666],geom->d_ICCELL[3][666666],molecs->ICREF[1],molecs->ICREF[2]);   
        // }
        if(N<=geom->NCCELLS && ((calc->FTIME-geom->d_CCELL[index(5,N,(geom->NCCELLS+1))]) > (geom->d_CCELL[index(3,N,(geom->NCCELLS+1))])))
        {
            //printf("%e %e\n",geom->d_CCELL[index(5,N,(geom->NCCELLS+1))],geom->d_CCELL[index(3,N,(geom->NCCELLS+1))]);
            DTC=2.e00*geom->d_CCELL[index(3,N,(geom->NCCELLS+1))];
            //calculate collisions appropriate to  time DTC
            if(geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]>1)
            {
                //no collisions calculated if there are less than two molecules in collision cell
                NN=geom->d_ICCELL[index(3,N,(geom->NCCELLS+1))];
                WFC=1.e00;
                if(geom->IWF==1 && geom->IVB==0)
                {
                    //dout
                    WFC=1.e00+geom->WFM*powf(geom->d_CELL[index(1,NN,(geom->NCELLS+1))],geom->IFX);
                }
                geom->d_CCELL[index(5,N,(geom->NCCELLS+1))]=geom->d_CCELL[index(5,N,(geom->NCCELLS+1))]+DTC;
                if(geom->IVB==0)
                {
                    AAA=geom->d_CCELL[index(1,N,(geom->NCCELLS+1))];
                }
                if(geom->IVB==1)
                {
                ...
                ASEL=0.5e00*geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]*(geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]-1)*WFC*calc->FNUM*geom->d_CCELL[index(4,N,(geom->NCCELLS+1))]*DTC/AAA+geom->d_CCELL[index(2,N,(geom->NCCELLS+1))];
                NSEL=ASEL;
                //dout
                geom->d_CCELL[index(2,N,(geom->NCCELLS+1))]=ASEL-(double)(NSEL);
                if(NSEL>0)
                {
                    ...
                    for(KL=1;KL<=NSEL;KL++)
                    {
                        ...
                        if(geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]==2)
                        {
                            K=1+geom->d_ICCELL[index(1,N,(geom->NCCELLS+1))];
                            //dout
                            L=molecs->ICREF[K];
                            K=2+geom->d_ICCELL[index(1,N,(geom->NCCELLS+1))];
                            //dout
                            M=molecs->ICREF[K];
                            if(M==molecs->IPCP[L])
                            {
                        ...
                        else
                        {
                            ...
                            //                            RANDOM_NUMBER(RANF);
                            calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                            K=(int)(calc->RANF*(double)(geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]))+geom->d_ICCELL[index(1,N,(geom->NCCELLS+1))]+1;
                            //dout
                            L=molecs->ICREF[K];
                            ...
                            if(calc->NNC==0)
                            {
                                ...
                                while(M==L)
                                {
                                    ...
                                    if(M==molecs->IPCP[L])
                                    {
                                        if(NTRY<5*geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))])
                                        {
                                            M=L;
                                        }
                                        else
                                        {
                                            III = 1;
                                            geom->d_CCELL[index(5,N,(geom->NCCELLS+1))]=geom->d_CCELL[index(5,N,(geom->NCCELLS+1))]-DTC/ASEL;
                                            M=L+1;
                                        }
                                    }
                                }
                            ...
                            else
                            {
                                ...
                                if(geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]<30)
                                {
                                    LL=geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))];
                                }
                                else
                                {
                                    LL=30;
                                ...
                                for(J=1;J<=LL;J++)
                                {
                                    if(LL<30)
                                    {
                                        K=J+geom->d_ICCELL[index(1,N,(geom->NCCELLS+1))];
                                    }
                                    else
                                    {
                                        //                                        RANDOM_NUMBER(RANF);
                                    ...
                                    if(MM != L)
                                    {
                                        ...
                                        if(MM != molecs->IPCP[L])
                                        {
                                            ...
                                            //dout
                                            A=fabsf(molecs->d_PX[index(1,L,(molecs->MNM+1))]-molecs->d_PX[index(1,MM,(molecs->MNM+1))]);
                        ...
                        if(III==0)
                        {
                            for(KK=1;KK<=3;KK++)
                            {
                                VRC[KK]=molecs->d_PV[index(KK,L,(molecs->MNM+1))]-molecs->d_PV[index(KK,M,(molecs->MNM+1))];
                            }
                            ...
                            {
                                //dout
                                CVR=VR*gas->CXSS*powf(2.e00*BOLTZ*gas->d_SP[index(2,1,(gas->MSP+1))]/(gas->RMAS*VRR),(gas->d_SP[index(3,1,(gas->MSP+1))]-0.5e00))*gas->RGFS;
                                if(CVR>geom->d_CCELL[index(4,N,(geom->NCCELLS+1))])
                                {
                                    geom->d_CCELL[index(4,N,(geom->NCCELLS+1))]=CVR;
                                }
                                //dout
                                //                                RANDOM_NUMBER(RANF);
                                calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                if(calc->RANF<CVR/geom->d_CCELL[index(4,N,(geom->NCCELLS+1))])
                                {
                                    ...
                                    calc->d_TCOL[index(1,1,(gas->MSP+1))]=calc->d_TCOL[index(1,1,(gas->MSP+1))]+2.e00;
                                    output->COLLS[NN]=output->COLLS[NN]+1.e000;
                                    output->WCOLLS[NN]=output->WCOLLS[NN]+WFC;
                                    //dout
                                    SEP=fabsf(molecs->d_PX[index(1,L,(molecs->MNM+1))]-molecs->d_PX[index(1,M,(molecs->MNM+1))]);
                                    output->CLSEP[NN]=output->CLSEP[NN]+SEP;
                                    if(gas->d_ISPR[index(1,1,(gas->MSP+1))]>0)
                                    {
                                        ...
                                        for(NSP=1;NSP<=2;NSP++)
                                        {
                                            ...
                                            if(gas->MMVM>0)
                                            {
                                                if(gas->ISPV[1]>0)
                                                {
                                                    ...
                                                    {
                                                        EVIB=(double)(molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]*BOLTZ*gas->d_SPVM[index(1,KV,1,(gas->MMVM+1),(gas->MSP+1))]);
                                                        ECC=ECT+EVIB;
                                                        if(gas->d_SPVM[index(3,KV,1,(gas->MMVM+1),(gas->MSP+1))]>0.0)
                                                        {
                                                            MAXLEV=ECC/(BOLTZ*gas->d_SPVM[index(1,KV,1,(gas->MMVM+1),(gas->MSP+1))]);
                                                            B=gas->d_SPVM[index(4,KV,1,(gas->MMVM+1),(gas->MSP+1))]/gas->d_SPVM[index(3,KV,1,(gas->MMVM+1),(gas->MSP+1))]; //Tdiss/Tref
                                                            A= gas->d_SPVM[index(4,KV,1,(gas->MMVM+1),(gas->MSP+1))]/output->d_VAR[index(8,NN,(geom->NCELLS+1))] ;//Tdiss/Ttrans
                                                            //ZV=(A**SPM(3,1,1))*(SPVM(3,KV,1)*(B**(-SPM(3,1,1))))**(((A**0.3333333D00)-1.D00)/((B**0.33333D00)-1.D00))
                                                            ZV=powf(A,gas->d_SPM[index(3,1,1,(gas->MSP+1),(gas->MSP+1))])*powf(gas->d_SPVM[index(3,KV,1,(gas->MMVM+1),(gas->MSP+1))]*powf(B,-gas->d_SPM[index(3,1,1,(gas->MSP+1),(gas->MSP+1))]),((powf(A,0.3333333e00)-1e00)/(powf(B,33333e00)-1.e00)));
                                                        }
                                                        else
                                                        {
                                                            ZV=gas->d_SPVM[index(2,KV,1,(gas->MMVM+1),(gas->MSP+1))];
                                                            MAXLEV=ECC/(BOLTZ*gas->d_SPVM[index(1,KV,1,(gas->MMVM+1),(gas->MSP+1))])+1;
                                                        }
                                                        //dout
                                                        //                                                        RANDOM_NUMBER(RANF);
                                                        calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                        if(1.e00/ZV>calc->RANF)
                                                        {
                                                            ...
                                                            while(II==0)
                                                            {
                                                                ...
                                                                molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]=IV;
                                                                EVIB=(double)(IV)*BOLTZ;
                                                                if(EVIB<ECC)
                                                                {
                                                                    PROB=powf((1.e00-EVIB/ECC),(1.5e00-gas->d_SPM[index(3,KV,1,(gas->MSP+1),(gas->MSP+1))]));
                                            ...
                                            if(gas->d_ISPR[index(1,1,(gas->MSP+1))] > 0)
                                            {
                                                if(gas->d_ISPR[index(2,1,(gas->MSP+1))]==0)
                                                {
                                                    B=1.e00/gas->d_SPR[index(1,1,(gas->MSP+1))];
                                                }
                                                else //use molecule rather than mean value
                                                {
                                                    B=1.e00/(gas->d_SPR[index(1,1,(gas->MSP+1))]+gas->d_SPR[index(2,1,(gas->MSP+1))]*output->d_VAR[index(8,NN,(geom->NCELLS+1))]+gas->d_SPR[index(3,1,(gas->MSP+1))]*powf(output->d_VAR[index(8,NN,(geom->NCELLS+1))],2));
                                                }
                                                //dout
                                                //                                                RANDOM_NUMBER(RANF);
                                                calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                if(B>calc->RANF)
                                                {
                                                    ...
                                                    if(gas->d_ISPR[index(1,1,(gas->MSP+1))]==2)
                                                    {
                                                        //dout
                                                        //                                                        RANDOM_NUMBER(RANF);
                                                        calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                        ERM=1.e00-powf(calc->RANF,1.e00/(2.5e00-gas->d_SP[index(3,1,(gas->MSP+1))])); //eqn(5.46)
                                                    }
                                                    else
                                                    {
                                                        //dout
                                                        LBS(globalState, 0.5e00*gas->d_ISPR[index(1,1,(gas->MSP+1))]-1.e00,1.5e00-gas->d_SP[index(3,1,(gas->MSP+1))],ERM);
                                                    }
                                        ...
                                        VR=sqrtf(2.e00*ECT/gas->d_SPM[index(1,1,1,(gas->MSP+1),(gas->MSP+1))]);
                                    }
                                    //end of L-B redistribution
                                    for(KK=1;KK<=3;KK++)
                                    {
                                        VCM[KK]=0.5e00*(molecs->d_PV[index(KK,L,(molecs->MNM+1))]+molecs->d_PV[index(KK,M,(molecs->MNM+1))]);
                                    }
                                    //dout
                                    if(fabsf(gas->d_SP[index(4,1,(gas->MSP+1))]-1.0) < 0.001)
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        //                                        RANDOM_NUMBER(RANF);
                                        calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                        B=2.e00*(powf(calc->RANF,gas->d_SP[index(4,1,(gas->MSP+1))]))-1.e00;
                                    ...
                                    for(KK=1;KK<=3;KK++)
                                    {
                                        molecs->d_PV[index(KK,L,(molecs->MNM+1))]=VCM[KK]+0.5e00*VRCP[KK];
                                        molecs->d_PV[index(KK,M,(molecs->MNM+1))]=VCM[KK]-0.5e00*VRCP[KK];
                                    }
                                    molecs->IPCP[L]=M;
                                    molecs->IPCP[M]=L;
                                }
                            ...
                            else
                            {
                                ...
                                // printf("%d %d \n",LS,MS);
                                CVR=VR*gas->d_SPM[index(2,LS,MS,(gas->MSP+1),(gas->MSP+1))]*powf(((2.e00*BOLTZ*gas->d_SPM[index(5,LS,MS,(gas->MSP+1),(gas->MSP+1))])/((gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))])*VRR)),(gas->d_SPM[index(3,LS,MS,(gas->MSP+1),(gas->MSP+1))]-0.5e00))*gas->d_SPM[index(6,LS,MS,(gas->MSP+1),(gas->MSP+1))];
                                if(CVR>geom->d_CCELL[index(4,N,(geom->NCCELLS+1))])
                                {
                                    geom->d_CCELL[index(4,N,(geom->NCCELLS+1))]=CVR;
                                }
                                ...
                                //                                    RANDOM_NUMBER(RANF);
                                calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                if(calc->RANF<(CVR/geom->d_CCELL[index(4,N,(geom->NCCELLS+1))]) && molecs->IPCELL[L]>0 && molecs->IPCELL[M]>0)
                                {
                                    //the collision occurs (-ve IPCELL indicates recombined molecule marled for removal)
                                    if(M==molecs->IPCP[L] && L==molecs->IPCP[M])
                                    {
                                        //file_9<<"Duplicate collision";
                                        printf("Duplicate Collision");
                                    }
                                    //atomicAdd(&calc->TOTCOL,1.e00);
                                    //calc->TOTCOL=calc->TOTCOL+1.e00;
                                    calc->COLL_TOTCOL[N]=calc->COLL_TOTCOL[N]+1.e00;
                                    calc->d_TCOL[index(LS,MS,(gas->MSP+1))]=calc->d_TCOL[index(LS,MS,(gas->MSP+1))]+1.e00;
                                    calc->d_TCOL[index(MS,LS,(gas->MSP+1))]=calc->d_TCOL[index(MS,LS,(gas->MSP+1))]+1.e00;
                                    output->COLLS[NN]=output->COLLS[NN]+1.e00;
                                    output->WCOLLS[NN]=output->WCOLLS[NN]+WFC;
                                    SEP=fabsf(molecs->d_PX[index(1,L,(molecs->MNM+1))]-molecs->d_PX[index(1,M,(molecs->MNM+1))]);
                                    output->CLSEP[NN]=output->CLSEP[NN]+SEP;
                                    RML=gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]/gas->d_SP[index(5,MS,(gas->MSP+1))];
                                    RMM=gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]/gas->d_SP[index(5,LS,(gas->MSP+1))];
                                    for(KK=1;KK<=3;KK++)
                                    {
                                        VCM[KK]=RML*molecs->d_PV[index(KK,L,(molecs->MNM+1))]+RMM*molecs->d_PV[index(KK,M,(molecs->MNM+1))];
                                    }
                                    IDISS=0;
                                    IREC=0;
                                    IEX=0;
                                    //check for dissociation
                                    if(gas->d_ISPR[index(1,LS,(gas->MSP+1))]>0 || gas->d_ISPR[index(1,MS,(gas->MSP+1))]>0)
                                    {
                                        ECT=0.5e00*gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]*VRR;
                                        for(NSP=1;NSP<=2;NSP++)
                                        {
                                            if(NSP==1)
                                            {
                                            ...
                                            if(gas->MMVM>0)
                                            {
                                                ...
                                                if(gas->ISPV[KS]>0)
                                                {
                                                    ...
                                                    {
                                                        if(molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]>=0 && IDISS==0)
                                                        {
                                                            //do not redistribute to a dissociating molecule marked for removal
                                                            EVIB=(double)(molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]*BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))]);
                                                            ECC=ECT+EVIB;
                                                            MAXLEV=ECC/(BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))]);
                                                            LIMLEV=gas->d_SPVM[index(4,KV,KS,(gas->MMVM+1),(gas->MSP+1))]/gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))];
                                                            if(MAXLEV > LIMLEV)
                                                            {
                                                                ...
                                                                //                                                                    RANDOM_NUMBER(RANF)
                                                                calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                                if(calc->RANF<gas->d_SPVM[index(5,KV,KS,(gas->MMVM+1),(gas->MSP+1))])
                                                                {
                                                                    IDISS=1;
                                                                    LZ=molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))];
                                                                    output->NDISSL[LZ]=output->NDISSL[LZ]+1;
                                                                    ECT=ECT-BOLTZ*gas->d_SPVM[index(4,KV,KS,(gas->MMVM+1),(gas->MSP+1))]+EVIB;
                                                                    //adjust VR for the change in energy
                                                                    VRR=2.e00*ECT/gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))];
                                                                    VR=sqrtf(VRR);
                                                                    molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]=-1;
                                    ...
                                    if(IDISS==0)
                                    {
                                        ...
                                        if(gas->d_ISPRC[index(LS,MS,(gas->MSP+1))]>0 && geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]>2)
                                        {
                                            ...
                                            AA=(PI/6.e00)*powf((gas->d_SP[index(1,LS,(gas->MSP+1))]+gas->d_SP[index(1,MS,(gas->MSP+1))]+gas->d_SP[index(1,KS,(gas->MSP+1))]),3); //reference volume
                                            BB=AA*gas->d_SPRC[index(1,LS,MS,KS,(gas->MSP+1),(gas->MSP+1),(gas->MSP+1))]*powf(output->d_VAR[index(8,NN,(geom->NCELLS+1))]/gas->d_SPVM[index(1,gas->d_ISPRK[index(LS,MS,(gas->MSP+1))],gas->d_ISPRC[index(LS,MS,(gas->MSP+1))],(gas->MMVM+1),(gas->MSP+1))],gas->d_SPRC[index(2,LS,MS,KS,(gas->MSP+1),(gas->MSP+1),(gas->MSP+1))]);//collision volume
                                            B=BB*geom->d_ICCELL[index(2,N,(geom->NCCELLS+1))]*calc->FNUM/AAA;
                                            if(B>1.e00)
                                            {
                                            ...
                                            {
                                                IREC=1;
                                                calc->TRECOMB[gas->d_ISPRC[index(LS,MS,(gas->MSP+1))]]=calc->TRECOMB[gas->d_ISPRC[index(LS,MS,(gas->MSP+1))]]+1.e00;
                                                //the collision now becomes a collision between these with L having the center of mass velocity
                                                A=0.5e00*gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]*VRR ;//the relative energy of the recombining molecules
                                                if(gas->d_ISPR[index(1,LS,(gas->MSP+1))]>0)
                                                    A=A+molecs->PROT[L];
                                                if(gas->MELE>1)
                                                    A=A+molecs->PELE[L];
                                                if(gas->ISPV[LS]>0)
                                                {
                                                    ...
                                                    {
                                                        JI=molecs->d_IPVIB[index(KVV,L,(molecs->MNM+1))];
                                                        if(JI<0)
                                                            JI=-JI;
                                                        if(JI==99999)
                                                            JI=0;
                                                        A=A+(double)(JI)*BOLTZ*gas->d_SPVM[index(1,KVV,LS,(gas->MMVM+1),(gas->MSP+1))];
                                                    }
                                                }
                                                if(gas->d_ISPR[index(1,MS,(gas->MSP+1))]>0)
                                                    A+=molecs->PROT[M];
                                                if(gas->MELE>1)
                                                    A=A+molecs->PELE[M];
                                                if(gas->ISPV[MS]>0)
                                                {
                                                    ...
                                                    {
                                                        JI=molecs->d_IPVIB[index(KVV,M,(molecs->MNM+1))];
                                                        if(JI<0)
                                                            JI=-JI;
                                                        if(JI==99999)
                                                            JI=0;
                                                        A=A+(double)(JI)*BOLTZ*gas->d_SPVM[index(1,KVV,MS,(gas->MMVM+1),(gas->MSP+1))];
                                                    }
                                                }
                                                gas->d_TREACL[index(2,LS,(gas->MSP+1))]=gas->d_TREACL[index(2,LS,(gas->MSP+1))]-1;
                                                gas->d_TREACL[index(2,MS,(gas->MSP+1))]=gas->d_TREACL[index(2,MS,(gas->MSP+1))]-1;
                                                LSI=LS;
                                                MSI=MS;
                                                LS=gas->d_ISPRC[index(LS,MS,(gas->MSP+1))];
                                                molecs->IPSP[L]=LS;
                                                ...
                                                if(NVM>IVM)
                                                {
                                                    ...
                                                    {
                                                        molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]=0;
                                                    }
                                                }
                                                if(gas->MELE>1)
                                                ...
                                                MS=KS;
                                                gas->d_TREACG[index(2,LS,(gas->MSP+1))]=gas->d_TREACG[index(2,LS,(gas->MSP+1))]+1;
                                                if(gas->d_ISPR[index(1,LS,(gas->MSP+1))]>0)
                                                {
                                                    molecs->PROT[L]=0.e00;
                                                }
                                                ...
                                                if(gas->ISPV[LS]>0)
                                                {
                                                    for(KVV=1;KVV<=gas->ISPV[LS];KVV++)
                                                    {
                                                        if(molecs->d_IPVIB[index(KVV,L,(molecs->MNM+1))]<0)
                                                        {
                                                            molecs->d_IPVIB[index(KVV,L,(molecs->MNM+1))]=-99999;
                                                        }
                                                        else
                                                        {
                                                            molecs->d_IPVIB[index(KVV,L,(molecs->MNM+1))]=0;
                                                        }
                                                    }
                                                }
                                                if(gas->d_ISPR[index(1,MS,(gas->MSP+1))]>0)
                                                ...
                                                if(gas->ISPV[MS]>0)
                                                {
                                                    for(KVV=1;KVV<=gas->ISPV[MS];KVV++)
                                                    {
                                                        molecs->d_IPVIB[index(KVV,M,(molecs->MNM+1))]=molecs->d_IPVIB[index(KVV,KT,(molecs->MNM+1))];
                                                    }
                                                }
                                                ECTOT=A+gas->d_SPVM[index(4,1,LS,(gas->MMVM+1),(gas->MSP+1))]*BOLTZ ; //the energy added to this collision
                                                for(KK=1;KK<=3;KK++)
                                                {
                                                    molecs->d_PV[index(KK,L,(molecs->MNM+1))]=VCM[KK];
                                                }
                                                for(KK=1;KK<=3;KK++)
                                                {
                                                    VRC[KK]=molecs->d_PV[index(KK,L,(molecs->MNM+1))]-molecs->d_PV[index(KK,M,(molecs->MNM+1))];
                                                ...
                                                VRR=VRC[1]*VRC[1]+VRC[2]*VRC[2]+VRC[3]*VRC[3];
                                                ECT=0.5e00*gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]*VRR*ECTOT;
                                                ...
                                                while(IK<0)
                                                {
                                                    ...
                                                    NK=NK+1;
                                                    BB=(output->d_VAR[index(8,NN,(geom->NCELLS+1))]-gas->d_SPRT[index(1,LSI,MSI,(gas->MSP+1),(gas->MSP+1))])*(gas->d_SPRP[index(2,LSI,MSI,NK,(gas->MSP+1),(gas->MSP+1),(gas->MVIBL+1))]-gas->d_SPRP[index(1,LSI,MSI,NK,(gas->MSP+1),(gas->MSP+1),(gas->MVIBL+1))])/(gas->d_SPRT[index(2,LSI,MSI,(gas->MSP+1),(gas->MSP+1))]-gas->d_SPRT[index(1,LSI,MSI,(gas->MSP+1),(gas->MSP+1))])-gas->d_SPRP[index(1,LSI,MSI,NK,(gas->MSP+1),(gas->MSP+1),(gas->MVIBL+1))];
                                                    if(calc->RANF<BB)
                                                        IK=NK;
                                                }
                                                molecs->d_IPVIB[index(1,L,(molecs->MNM+1))]=IK;
                                                ECT=ECT-(double)(IK)*BOLTZ*gas->d_SPVM[index(1,gas->d_ISPRK[index(LSI,MSI,(gas->MSP+1))],LS,(gas->MMVM+1),(gas->MSP+1))];
                                                VRR=2.e00*ECT/gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))];
                                                VR=sqrtf(VRR);
                                                RML=gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]/gas->d_SP[index(5,MS,(gas->MSP+1))];
                                                RMM=gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]/gas->d_SP[index(5,LS,(gas->MSP+1))];
                                        ...
                                        if(gas->d_NSPEX[index(LS,MS,(gas->MSP+1))]>0 && IREC==0 && IDISS==0)
                                        {
                                            ...
                                            for(JJ=1;JJ<=gas->d_NSPEX[index(LS,MS,(gas->MSP+1))];JJ++)
                                            {
                                                if(LS==gas->d_ISPEX[index(JJ,1,LS,MS,8,(gas->MSP+1),(gas->MSP+1))])
                                                {
                                                    K=L; KS=LS;JS=MS;
                                                }
                                                else
                                                ...
                                                if(gas->d_SPEX[index(3,JJ,LS,MS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]<0.e00)
                                                    KV=gas->d_ISPEX[index(JJ,5,LS,MS,8,(gas->MSP+1),(gas->MSP+1))];
                                                if(gas->d_SPEX[index(3,JJ,LS,MS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]>0.e00)
                                                {
                                                    KV=gas->d_ISPEX[index(JJ,7,LS,MS,8,(gas->MSP+1),(gas->MSP+1))];
                                                }
                                                JI=molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))];
                                                if(JI<0)
                                                    JI=-JI;
                                                if(JI==99999)
                                                    JI=0;
                                                ECC=0.5e00*gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]*VRR+(double)(JI)*BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))];
                                                if(gas->d_SPEX[index(3,JJ,KS,JS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]>0.e00)
                                                {
                                                    //reverse exothermic reaction
                                                    gas->PSF[JJ]=(gas->d_SPEX[index(1,JJ,KS,JS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]*powf(output->d_VAR[index(8,NN,(geom->NCELLS+1))]/273.e00,gas->d_SPEX[index(2,JJ,KS,JS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]))*expf(-gas->d_SPEX[index(6,JJ,KS,JS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]/(BOLTZ*output->d_VAR[index(8,NN,(geom->NCELLS+1))]));
                                                }
                                                else
                                                {
                                                    //forward endothermic reaction
                                                    MAXLEV=ECC/(BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))]);
                                                    EA=fabsf(gas->d_SPEX[index(3,JJ,KS,JS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]); //temporarily just the heat of reaction;
                                                    if(ECC>EA)
                                                    {
                                                        //the collision energy must exceed the heat of reaction
                                                        EA=EA+gas->d_SPEX[index(6,JJ,KS,JS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]; //the activation energy now includes the energy barrier
                                                        DEN=0.e00;
                                                        for(IAX=0;IAX<=MAXLEV;IAX++)
                                                        {
                                                            DEN=DEN+powf((1.e00-(double)(IAX)*BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))]/ECC),(1.5e00-gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))]));
                                                        }
                                                        gas->PSF[JJ]=(double)(gas->d_ISPEX[index(JJ,6,LS,MS,8,(gas->MSP+1),(gas->MSP+1))])*powf((1.e00-EA/ECC),(1.5e00-gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))]))/DEN;
                                                    }
                                                }
                                            }
                                            if(gas->d_NSPEX[index(LS,MS,(gas->MSP+1))]>1)
                                            {
                                                BB=0.e00;
                                                for(JJ=1;JJ<=gas->d_NSPEX[index(LS,MS,(gas->MSP+1))];JJ++)
                                                {
                                                ...
                                                if(BB>calc->RANF)
                                                {
                                                    ...
                                                    JJ=0;
                                                    //NTRY=0;
                                                    while(JJ<gas->d_NSPEX[index(LS,MS,(gas->MSP+1))] && IEX==0)
                                                    {
                                                        // NTRY=NTRY+1;
                                                        // if(NTRY>100)
                                                        // {
                                            ...
                                            if(IEX>0)
                                            {
                                                ...
                                                JX=gas->d_NEX[index(IEX,LS,MS,(gas->MSP+1),(gas->MSP+1))];
                                                //cout<<"Reaction"<<JX;
                                                gas->TNEX[JX]=gas->TNEX[JX]+1.e00;
                                                //cout<<IEX<<L<<M<<LS<<MS;
                                                molecs->IPSP[L]=gas->d_ISPEX[index(IEX,3,LS,MS,8,(gas->MSP+1),(gas->MSP+1))]; //L is now the new molecule that splits
                                                molecs->IPSP[M]=gas->d_ISPEX[index(IEX,4,LS,MS,8,(gas->MSP+1),(gas->MSP+1))];
                                                LSI=LS;
                                                MSI=MS;
                                                //any additional vibrational modes must be set to zero
                                                IVM=gas->ISPV[LS];
                                                ...
                                                if(NVM>IVM)
                                                {
                                                    for(KV=IVM+1;KV<=NVM;KV++)
                                                    {
                                                        molecs->d_IPVIB[index(KV,M,(molecs->MNM+1))]=0;
                                                    }
                                                }
                                                ...
                                                if(gas->ISPV[LS]>0)
                                                {
                                                    for(KV=1;KV<=gas->ISPV[LS];KV++)
                                                    {
                                                        JI=molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))];
                                                        if(JI<0)
                                                            JI=-JI;
                                                        if(JI==99999)
                                                            JI=0;
                                                        ECT=ECT+(double)(JI)*BOLTZ*gas->d_SPVM[index(1,KV,LS,(gas->MMVM+1),(gas->MSP+1))];
                                                    }
                                                }
                                                if(gas->d_ISPR[index(1,MS,(gas->MSP+1))]>0)
                                                ...
                                                if(gas->ISPV[MS]>0)
                                                {
                                                    for(KV=1;KV<=gas->ISPV[MS];KV++)
                                                    {
                                                        JI=molecs->d_IPVIB[index(KV,M,(molecs->MNM+1))];
                                                        if(JI<0)
                                                            JI=-JI;
                                                        if(JI==99999)
                                                            JI=0;
                                                        ECT=ECT+(double)(JI)*BOLTZ*gas->d_SPVM[index(1,KV,MS,(gas->MMVM+1),(gas->MSP+1))];
                                                    }
                                                }
                                                ECT=ECT+gas->d_SPEX[index(3,IEX,LS,MS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))];
                                                if(ECT<0.0)
                                                {
                                                    printf ("-VE ECT %f\n",ECT);
                                                ...
                                                if(gas->d_SPEX[index(3,IEX,LS,MS,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]<0.e00)
                                                {
                                                    gas->d_TREACL[index(3,LS,(gas->MSP+1))]=gas->d_TREACL[index(3,LS,(gas->MSP+1))]-1;
                                                    gas->d_TREACL[index(3,MS,(gas->MSP+1))]=gas->d_TREACL[index(3,MS,(gas->MSP+1))]-1;
                                                    LS=molecs->IPSP[L] ;
                                                    MS=molecs->IPSP[M] ;
                                                    gas->d_TREACG[index(3,LS,(gas->MSP+1))]=gas->d_TREACG[index(3,LS,(gas->MSP+1))]+1;
                                                    gas->d_TREACG[index(3,MS,(gas->MSP+1))]=gas->d_TREACG[index(3,MS,(gas->MSP+1))]+1;
                                                }
                                                else
                                                {
                                                    gas->d_TREACL[index(4,LS,(gas->MSP+1))]=gas->d_TREACL[index(4,LS,(gas->MSP+1))]-1;
                                                    gas->d_TREACL[index(4,MS,(gas->MSP+1))]=gas->d_TREACL[index(4,MS,(gas->MSP+1))]-1;
                                                    LS=molecs->IPSP[L] ;
                                                    MS=molecs->IPSP[M] ;
                                                    gas->d_TREACG[index(4,LS,(gas->MSP+1))]=gas->d_TREACG[index(4,LS,(gas->MSP+1))]+1;
                                                    gas->d_TREACG[index(4,MS,(gas->MSP+1))]=gas->d_TREACG[index(4,MS,(gas->MSP+1))]+1;
                                                }
                                                RML=gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]/gas->d_SP[index(5,MS,(gas->MSP+1))];
                                                RMM=gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]/gas->d_SP[index(5,LS,(gas->MSP+1))];
                                                ...
                                                if(gas->ISPV[LS]>0)
                                                {
                                                    ...
                                                    {
                                                        if(molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]<0)
                                                        {
                                                            molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]=-99999;
                                                        }
                                                        else
                                                        {
                                                            molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]=0;
                                                ...
                                                if(gas->ISPV[MS]>0)
                                                {
                                                    ...
                                                    {
                                                        if(molecs->d_IPVIB[index(KV,M,(molecs->MNM+1))]<0)
                                                        {
                                                            molecs->d_IPVIB[index(KV,M,(molecs->MNM+1))]=-99999;
                                                        }
                                                        else
                                                        {
                                                            molecs->d_IPVIB[index(KV,M,(molecs->MNM+1))]=0;
                                                ...
                                                if(gas->d_SPEX[index(3,IEX,LSI,MSI,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]>0.e00)
                                                {
                                                    ...
                                                    while(IK<0)
                                                    {
                                                        ...
                                                        // }
                                                        NK=NK+1;
                                                        //must edit for MMVM>0
                                                        BB=(output->d_VAR[index(8,NN,(geom->NCELLS+1))]-gas->d_SPEX[index(4,IEX,LSI,MSI,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))])*  (gas->SPREX[2][IEX][LSI][MSI][NK]-gas->SPREX[1][IEX][LSI][MSI][NK])/(gas->d_SPEX[index(5,IEX,LSI,MSI,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))]-gas->d_SPEX[index(4,IEX,LSI,MSI,(gas->MMEX+1),(gas->MSP+1),(gas->MSP+1))])+gas->SPREX[1][IEX][LSI][MSI][NK];
                                                        if(calc->RANF<BB)
                                                            IK=NK;
                                                    }
                                                    if(gas->d_NSLEV[index(1,LS,(gas->MSP+1))]>0)
                                                    {
                                                        IK+=gas->d_NSLEV[index(1,LS,(gas->MSP+1))];
                                                        gas->d_NSLEV[index(1,LS,(gas->MSP+1))]=0;
                                                    }
                                                    KV=gas->d_ISPEX[index(IEX,7,LSI,MSI,8,(gas->MSP+1),(gas->MSP+1))];
                                                    molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]=IK;
                                                    EVIB=(double)(IK)*BOLTZ*gas->d_SPVM[index(1,KV,LS,(gas->MMVM+1),(gas->MSP+1))];
                                                    ECT=ECT-EVIB;
                                                    if(ECT<0.e00)
                                                    {
                                                        //NTRY=0;
                                                        while(ECT<0.e00)
                                                        {
                                                            ...
                                                            molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]=molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]-1;
                                                            gas->d_NSLEV[index(1,LS,(gas->MSP+1))]+=1;
                                                            ECT=ECT+BOLTZ*gas->d_SPVM[index(1,KV,LS,(gas->MMVM+1),(gas->MSP+1))];
                                                        }
                                                    }
                                                }
                                                else
                                                {
                                                    ...
                                                    KV=gas->d_ISPEX[index(IEX,5,LS,MS,8,(gas->MSP+1),(gas->MSP+1))];
                                                    //dout
                                                    SVIB(globalState, LS,output->d_VAR[index(8,NN,(geom->NCELLS+1))],IK,KV,gas,calc);
                                                    if(gas->d_NSLEV[index(2,LS,(gas->MSP+1))]>0)
                                                    {
                                                        IK=IK+gas->d_NSLEV[index(2,LS,(gas->MSP+1))];
                                                        gas->d_NSLEV[index(2,LS,(gas->MSP+1))]=0;
                                                    }
                                                    molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]=IK;
                                                    EVIB=(double)(IK)*BOLTZ*gas->d_SPVM[index(1,KV,LS,(gas->MMVM+1),(gas->MSP+1))];
                                                    ECT=ECT-EVIB;
                                                    if(ECT<0.e00)
                                                    {
                                                        ...
                                                        {
                                                            //NTRY+=1;
                                                            molecs->d_IPVIB[index(KV,L,(molecs->MNM+1))]-=1;
                                                            gas->d_NSLEV[index(2,LS,(gas->MSP+1))]+=1;
                                                            ECT=ECT+BOLTZ*gas->d_SPVM[index(1,KV,LS,(gas->MMVM+1),(gas->MSP+1))];
                                                            // if(NTRY>100)
                                                            // {
                                                            //cout<<"NTRY ECT<0#2"<<NTRY<<endl;
                                                            // molecs->d_IPVIB[KV][L]=0;
                                                            //   ECT+=EVIB;
                                                            //   gas->d_NSLEV[2][LS]=0;
                                                            // }
                                                        }
                                                ...
                                                SROT(globalState, LS,output->d_VAR[index(8,NN,(geom->NCELLS+1))],molecs->PROT[L],gas,calc);
                                    ...
                                    if(IREC==0 && IDISS==0)
                                    {
                                        ...
                                        for(NSP=1;NSP<=2;NSP++)
                                        {
                                            ...
                                            if(gas->MELE>1)
                                            {
                                                B=1.e00/gas->d_QELC[index(3,1,KS,(gas->MELE+1),(gas->MSP+1))];
                                                //dout
                                                //                                                        RANDOM_NUMBER(RANF);
                                                calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                if(B>calc->RANF)
                                                {
                                                    ...
                                                    if(gas->NELL[KS]==1){
                                                        NPS=gas->d_QELC[index(1,1,KS,(gas->MELE+1),(gas->MSP+1))]; //number of possible states is at least the degeneracy of the ground state
                                                    }
                                                    if(gas->NELL[KS]>1)
                                                    {
                                                        for(NEL=1;NEL<=gas->NELL[KS];NEL++)
                                                        {
                                                            if(ECC>BOLTZ*gas->d_QELC[index(2,NEL,KS,(gas->MELE+1),(gas->MSP+1))])
                                                                NPS=NPS+gas->d_QELC[index(1,NEL,KS,(gas->MELE+1),(gas->MSP+1))];
                                                        }
                                                        II=0;
                                                        //NTRY=0;
                                                        while(II==0)
                                                        {
                                                            ...
                                                            for(NEL=1;NEL<=gas->NELL[KS];NEL++)
                                                            {
                                                                NAS= NAS+gas->d_QELC[index(1,NEL,KS,(gas->MELE+1),(gas->MSP+1))];
                                                            ...
                                                            //                                                                    RANDOM_NUMBER(RANF);
                                                            calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                            if((1.e00/(B*gas->d_QELC[index(3,NLEVEL,KS,(gas->MELE+1),(gas->MSP+1))]))<calc->RANF)
                                                            {
                                                                II=1;
                                                            }
                                                            else
                                                            {
                                                                if(ECC>BOLTZ*gas->d_QELC[index(2,NLEVEL,KS,(gas->MELE+1),(gas->MSP+1))])
                                                                {
                                                                    PROB=powf(1.e00-BOLTZ*gas->d_QELC[index(2,NLEVEL,KS,(gas->MELE+1),(gas->MSP+1))]/ECC,(1.5e00-gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))]));
                                                                    //dout
                                                                    //                                                                            RANDOM_NUMBER(RANF);
                                                                    calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                                    if(PROB>calc->RANF)
                                                                    {
                                                                        II=1;
                                                                        molecs->PELE[K]=BOLTZ*gas->d_QELC[index(2,NLEVEL,KS,(gas->MELE+1),(gas->MSP+1))];
                                                                    }
                                                                }
                                                            }
                                                        }
                                            ...
                                            if(gas->MMVM>0 && IEX==0)
                                            {
                                                if(gas->ISPV[KS]>0)
                                                {
                                                    for(KV=1;KV<=gas->ISPV[KS];KV++)
                                                    {
                                                        if(molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]>=0 && IDISS==0) //do not redistribute to a dissociating molecule marked for removal
                                                        {
                                                            EVIB=(double)(molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))])*BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))];
                                                            ECC=ECT+EVIB;
                                                            MAXLEV=ECC/(BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))]);
                                                            if(gas->d_SPVM[index(3,KV,KS,(gas->MMVM+1),(gas->MSP+1))]>0.0)
                                                            {   
                                                                B=gas->d_SPVM[index(4,KV,KS,(gas->MMVM+1),(gas->MSP+1))]/gas->d_SPVM[index(3,KV,KS,(gas->MMVM+1),(gas->MSP+1))];
                                                                A=gas->d_SPVM[index(4,KV,KS,(gas->MMVM+1),(gas->MSP+1))]/output->d_VAR[index(8,NN,(geom->NCELLS+1))];
                                                               ZV = powf(A,gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))])*powf((gas->d_SPVM[index(2,KV,KS,(gas->MMVM+1),(gas->MSP+1))]*powf(B,-gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))])),((powf(A,0.3333333e00)-1.e00)/(powf(B,0.33333e00)-1.e00)));
                                                               
                                                            }
                                                            else
                                                                ZV=gas->d_SPVM[index(2,KV,KS,(gas->MMVM+1),(gas->MSP+1))];
                                                            ...
                                                            if(1.e00/ZV>calc->RANF ||IREC==1)
                                                            {
                                                                ...
                                                                while(II==0 && NSTEP<100000)
                                                                {
                                                                    ...
                                                                    IV=calc->RANF*(MAXLEV+0.99999999e00);
                                                                    molecs->d_IPVIB[index(KV,K,(molecs->MNM+1))]=IV;
                                                                    EVIB=(double)(IV)*BOLTZ*gas->d_SPVM[index(1,KV,KS,(gas->MMVM+1),(gas->MSP+1))];
                                                                    if(EVIB<ECC)
                                                                    {
                                                                        PROB=powf(1.e00-EVIB/ECC,1.5e00-gas->d_SPVM[index(3,KV,JS,(gas->MMVM+1),(gas->MSP+1))]);
                                                                        //PROB is the probability ratio of eqn (3.28)
                                                                        //                                                                                RANDOM_NUMBER(RANF);
                                                                        calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                                        if(PROB>calc->RANF)
                                            ...
                                            //now rotation of this molecule
                                            //dout
                                            if(gas->d_ISPR[index(1,KS,(gas->MSP+1))] > 0)
                                            {
                                                if(gas->d_ISPR[index(2,KS,(gas->MSP+1))]==0 && gas->d_ISPR[index(2,JS,(gas->MSP+1))]==0)
                                                {
                                                    B=1.e00/gas->d_SPM[index(7,KS,JS,(gas->MSP+1),(gas->MSP+1))];
                                                }
                                                else
                                                    B=1.e00/(gas->d_SPR[index(1,KS,(gas->MSP+1))])+gas->d_SPR[index(2,KS,(gas->MSP+1))]*output->d_VAR[index(8,NN,(geom->NCELLS+1))]+gas->d_SPR[index(3,KS,(gas->MSP+1))]*powf(output->d_VAR[index(8,NN,(geom->NCELLS+1))],2);
                                                //                                                        RANDOM_NUMBER(RANF);
                                                calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                if(B>calc->RANF|| IREC==1)
                                                {
                                                    ECC=ECT+molecs->PROT[K];
                                                    if(gas->d_ISPR[index(1,KS,(gas->MSP+1))]==2)
                                                    {
                                                        //                                                                RANDOM_NUMBER(RANF);
                                                        calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                                        ERM=1.e00-powf(calc->RANF,(1.e00/(2.5e00-gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))])));//eqn(5.46)
                                                    }
                                                    else
                                                        LBS(globalState, 0.5e00*gas->d_ISPR[index(1,KS,(gas->MSP+1))]-1.e00,1.5e00-gas->d_SPM[index(3,KS,JS,(gas->MSP+1),(gas->MSP+1))],ERM);
                                                    molecs->PROT[K]=ERM*ECC;
                                                    ECT=ECC-molecs->PROT[K];
                                                }
                                            }
                                        ...
                                        //adjust VR for the change in energy
                                        VR=sqrtf(2.e00*ECT/gas->d_SPM[index(1,LS,MS,(gas->MSP+1),(gas->MSP+1))]);
                                    }//end of L-B redistribution
                                    if(fabsf(gas->d_SPM[index(8,LS,MS,(gas->MSP+1),(gas->MSP+1))]-1.0)<0.001)
                                    {
                                        //use the VHS logic
                                        //                                                RANDOM_NUMBER(RANF);
                                        calc->RANF=generate(globalState, 0);//((double)rand()/(double)RAND_MAX);
                                    ...
                                    for(KK=1;KK<=3;KK++)
                                    {
                                        molecs->d_PV[index(KK,L,(molecs->MNM+1))]=VCM[KK]+RMM*VRCP[KK];
                                        molecs->d_PV[index(KK,M,(molecs->MNM+1))]=VCM[KK]-RMM*VRCP[KK];
                                    }
                                    molecs->IPCP[L]=M;
                                    molecs->IPCP[M]=L;
                                    //call energy(0,E2)
...
{   
    ...
    //4,gas->MELE+1,gas->MSP+1,
    //allocating kernel parameters
    cudaMalloc(&geom->d_ICCELL, 4*(geom->NCCELLS+1)*sizeof(int));
    cudaMalloc(&geom->d_CCELL, 6*(geom->NCCELLS+1)*sizeof(double));
    cudaMalloc(&geom->d_CELL, 5*(geom->NCELLS+1)*sizeof(double));
    cudaMalloc(&molecs->d_PX, (calc->NCLASS+1)*(molecs->MNM+1)*sizeof(double));
    cudaMalloc(&molecs->d_PV, (4)*(molecs->MNM+1)*sizeof(double));
    cudaMalloc(&gas->d_SP, (7)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&gas->d_SPR, (4)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&gas->d_ISPR, (3)*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&output->d_VAR, 24*(geom->NCELLS+1)*sizeof(double));
    cudaMalloc(&gas->d_SPM, (9)*(gas->MSP+1)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&calc->d_TCOL, (gas->MSP+1)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&gas->d_ISPRC, (gas->MSP+1)*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_SPRC, 5*(gas->MSP+1)*(gas->MSP+1)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&gas->d_TREACG, 5*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_TREACL, 5*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_SPRT, 3*(gas->MSP+1)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&gas->d_SPRP, 3*(gas->MSP+1)*(gas->MSP+1)*(gas->MVIBL+1)*sizeof(double));
    cudaMalloc(&gas->d_ISPRK, (gas->MSP+1)*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_NSPEX, (gas->MSP+1)*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_ISPEX, (gas->MMEX+1)*8*(gas->MSP+1)*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_SPEX, 7*(gas->MMEX+1)*(gas->MSP+1)*(gas->MSP+1)*sizeof(double));
    cudaMalloc(&gas->d_NEX, (gas->MMEX+1)*(gas->MSP+1)*(gas->MSP+1)*sizeof(int));
   cudaMalloc(&gas->SPREX, 3*(gas->MMEX+1)*(gas->MSP+1)*(gas->MSP+1)*(gas->MVIBL+1)*sizeof(double));
    cudaMalloc(&gas->d_NSLEV, (3)*(gas->MSP+1)*sizeof(int));
    cudaMalloc(&gas->d_QELC, 4*(gas->MELE+1)*(gas->MSP+1)*sizeof(double));
    
    if(gas->MMVM>0){
        cudaMalloc(&gas->d_SPVM, (6)*(gas->MMVM+1)*(gas->MSP+1)*sizeof(double));
        cudaMalloc(&molecs->d_IPVIB, (gas->MMVM+1)*(molecs->MNM+1)*sizeof(int));
        for(int i=0;i<(gas->MMVM+1);i++){
            cudaMemcpy(molecs->d_IPVIB+i*(molecs->MNM+1), molecs->IPVIB[i], (molecs->MNM+1)*sizeof(int), cudaMemcpyHostToDevice);
        }
        for(int i=0;i<6;i++){
            for(int j=0;j<(gas->MMVM+1);j++)
                cudaMemcpy(gas->d_SPVM+(j+i*(gas->MMVM+1))*(gas->MSP+1), gas->SPVM[i][j], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
        }
    }

    for(int i=0;i<9;i++){
        for(int j=0;j<(gas->MSP+1);j++)
            cudaMemcpy(gas->d_SPM+(j+i*(gas->MSP+1))*(gas->MSP+1), gas->SPM[i][j], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
    }
    for(int i=0;i<3;i++){
        for(int j=0;j<(gas->MSP+1);j++)
            cudaMemcpy(gas->d_SPRT+(j+i*(gas->MSP+1))*(gas->MSP+1), gas->SPRT[i][j], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
    }
    for(int i=0;i<(gas->MMEX+1);i++){
        for(int j=0;j<(gas->MSP+1);j++)
            cudaMemcpy(gas->d_NEX+(j+i*(gas->MSP+1))*(gas->MSP+1), gas->NEX[i][j], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
    }
    for(int i=0;i<4;i++){
        for(int j=0;j<(gas->MELE+1);j++)
            cudaMemcpy(gas->d_QELC+(j+i*(gas->MELE+1))*(gas->MSP+1), gas->QELC[i][j], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
    }

    for(int i=0;i<5;i++){
        for(int j=0;j<(gas->MSP+1);j++){
            for(int k=0;k<(gas->MSP+1);k++)
                cudaMemcpy(gas->d_SPRC+(k+(gas->MSP+1)*(j+i*(gas->MSP+1)))*(gas->MSP+1), gas->SPRC[i][j][k], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
        }
    }
    for(int i=0;i<3;i++){
        for(int j=0;j<(gas->MSP+1);j++){
            for(int k=0;k<(gas->MSP+1);k++)
                cudaMemcpy(gas->d_SPRP+(k+(gas->MSP+1)*(j+i*(gas->MSP+1)))*(gas->MVIBL+1), gas->SPRP[i][j][k], (gas->MVIBL+1)*sizeof(double), cudaMemcpyHostToDevice);
        }
    }
    for(int i=0;i<(gas->MMEX+1);i++){
        for(int j=0;j<8;j++){
            for(int k=0;k<(gas->MSP+1);k++)
                cudaMemcpy(gas->d_ISPEX+(k+(gas->MSP+1)*(j+i*8))*(gas->MSP+1), gas->ISPEX[i][j][k], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        }
    }
    for(int i=0;i<7;i++){
        for(int j=0;j<(gas->MMEX+1);j++){
            for(int k=0;k<(gas->MSP+1);k++)
                cudaMemcpy(gas->d_SPEX+(k+(gas->MSP+1)*(j+i*(gas->MMEX+1)))*(gas->MSP+1), gas->SPEX[i][j][k], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
        }
    }
    ...
    for(int i=0;i<3;i++){
        for(int j=0;j<(gas->MMEX+1);j++){
            for(int k=0;k<(gas->MSP+1);k++){
                for(int l=0;l<(gas->MSP+1);l++){
                     cudaMemcpy(gas->d_SPREX+(i*(gas->MMEX+1)*(gas->MSP+1)*(gas->MSP+1)*(gas->MVIBL+1) + j*(gas->MSP+1)*(gas->MSP+1)*(gas->MVIBL+1) + k*(gas->MSP+1)*(gas->MVIBL+1) + l*(gas->MVIBL+1)), gas->SPREX[i][j][k][l], (gas->MVIBL+1)*sizeof(double), cudaMemcpyHostToDevice);
                }
            }
        }
    }
    ...
    for(int i=0;i<24;i++){
        if(i<3)
            cudaMemcpy(gas->d_ISPR+i*(gas->MSP+1), gas->ISPR[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        if(i<3)
            cudaMemcpy(gas->d_NSLEV+i*(gas->MSP+1), gas->NSLEV[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        if(i<4)
            cudaMemcpy(gas->d_SPR+i*(gas->MSP+1), gas->SPR[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        if(i<4)
            cudaMemcpy(geom->d_ICCELL+i*(geom->NCCELLS+1), geom->ICCELL[i], (geom->NCCELLS+1)*sizeof(int), cudaMemcpyHostToDevice);
        if(i<4)
            cudaMemcpy(molecs->d_PV+i*(molecs->MNM+1), molecs->PV[i], (molecs->MNM+1)*sizeof(double), cudaMemcpyHostToDevice);
        if(i<5)
            cudaMemcpy(geom->d_CELL+i*(geom->NCELLS+1), geom->CELL[i], (geom->NCELLS+1)*sizeof(double), cudaMemcpyHostToDevice);
        if(i<5)
            cudaMemcpy(gas->d_TREACL+i*(gas->MSP+1), gas->TREACL[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        if(i<5)
            cudaMemcpy(gas->d_TREACG+i*(gas->MSP+1), gas->TREACG[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        if(i<6)
            cudaMemcpy(geom->d_CCELL+i*(geom->NCCELLS+1), geom->CCELL[i], (geom->NCCELLS+1)*sizeof(double), cudaMemcpyHostToDevice);
        if(i<7)
            cudaMemcpy(gas->d_SP+i*(gas->MSP+1), gas->SP[i], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
        if(i<24)
            cudaMemcpy(output->d_VAR+i*(geom->NCELLS+1), output->VAR[i], (geom->NCELLS+1)*sizeof(double), cudaMemcpyHostToDevice);
    }
    for(int i=0;i<calc->NCLASS+1;i++){
        cudaMemcpy(molecs->d_PX+i*(molecs->MNM+1), molecs->PX[i], (molecs->MNM+1)*sizeof(double), cudaMemcpyHostToDevice);
    }
    for(int i=0;i<(gas->MSP+1);i++){
        cudaMemcpy(calc->d_TCOL+i*(gas->MSP+1), calc->TCOL[i], (gas->MSP+1)*sizeof(double), cudaMemcpyHostToDevice);
        cudaMemcpy(gas->d_ISPRC+i*(gas->MSP+1), gas->ISPRC[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(gas->d_ISPRK+i*(gas->MSP+1), gas->ISPRK[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(gas->d_NSPEX+i*(gas->MSP+1), gas->NSPEX[i], (gas->MSP+1)*sizeof(int), cudaMemcpyHostToDevice);
    }
    

    memset(calc->COLL_TOTCOL,0.e00,(N+1)*sizeof(double));
    // printf("start\n %e %e\n %e %e\n %e %e\n ",geom->XB[1],geom->XB[2], geom->CCELL[5][666], geom->CCELL[5][666666],geom->CCELL[3][666], geom->CCELL[3][666666]);
    // printf("%d %d\n %d %d\n %d %d\n",geom->ICCELL[2][666], geom->ICCELL[2][666666],geom->ICCELL[3][666],geom->ICCELL[3][666666],molecs->ICREF[1],molecs->ICREF[2]);
    ...
    if(gas->MMVM>0){
        for(int i=0;i<(gas->MMVM+1);i++){
            cudaMemcpy(molecs->IPVIB[i], molecs->d_IPVIB+i*(molecs->MNM+1), (molecs->MNM+1)*sizeof(int), cudaMemcpyDeviceToHost);
        }
        // for(int i=0;i<6;i++){
        //     for(int j=0;j<(gas->MMVM+1);j++)
        //         cudaMemcpy(gas->SPVM[i][j], gas->d_SPVM+(j+i*(gas->MMVM+1))*(gas->MSP+1),(gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
        // }
    }

    // for(int i=0;i<9;i++){
    //     for(int j=0;j<(gas->MSP+1);j++)
    //         cudaMemcpy(gas->SPM[i][j], gas->d_SPM+(j+i*(gas->MSP+1))*(gas->MSP+1), (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
    // }
    // for(int i=0;i<3;i++){
    //     for(int j=0;j<(gas->MSP+1);j++)
    //         cudaMemcpy(gas->SPRT[i][j], gas->d_SPRT+(j+i*(gas->MSP+1))*(gas->MSP+1), (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
    // }
    // for(int i=0;i<(gas->MMEX+1);i++){
    //     for(int j=0;j<(gas->MSP+1);j++)
    //         cudaMemcpy(gas->NEX[i][j], gas->d_NEX+(j+i*(gas->MSP+1))*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
    // }
    // for(int i=0;i<4;i++){
    //     for(int j=0;j<(gas->MELE+1);j++)
    //         cudaMemcpy(gas->QELC[i][j], gas->d_QELC+(j+i*(gas->MELE+1))*(gas->MSP+1), (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
    // }

    // for(int i=0;i<5;i++){
    //     for(int j=0;j<(gas->MSP+1);j++){
    //         for(int k=0;k<(gas->MSP+1);k++)
    //             cudaMemcpy(gas->SPRC[i][j][k], gas->d_SPRC+(k+(gas->MSP+1)*(j+i*(gas->MSP+1)))*(gas->MSP+1), (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
    //     }
    // }
    // for(int i=0;i<3;i++){
    //     for(int j=0;j<(gas->MSP+1);j++){
    //         for(int k=0;k<(gas->MSP+1);k++)
    //             cudaMemcpy(gas->SPRP[i][j][k], gas->d_SPRP+(k+(gas->MSP+1)*(j+i*(gas->MSP+1)))*(gas->MVIBL+1), (gas->MVIBL+1)*sizeof(double), cudaMemcpyDeviceToHost);
    //     }
    // }
    // for(int i=0;i<(gas->MMEX+1);i++){
    //     for(int j=0;j<8;j++){
    //         for(int k=0;k<(gas->MSP+1);k++)
    //             cudaMemcpy(gas->ISPEX[i][j][k], gas->d_ISPEX+(k+(gas->MSP+1)*(j+i*8))*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
    //     }
    // }
    // for(int i=0;i<7;i++){
    //     for(int j=0;j<(gas->MMEX+1);j++){
    //         for(int k=0;k<(gas->MSP+1);k++)
    //             cudaMemcpy(gas->SPEX[i][j][k], gas->d_SPEX+(k+(gas->MSP+1)*(j+i*(gas->MMEX+1)))*(gas->MSP+1), (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
    //     }
    // }

    // cout<<gas->SPREX[2][gas->MMEX][gas->MSP][gas->MSP][gas->MVIBL]<<endl;
    // for(int i=0;i<3;i++){
    //     for(int j=0;j<(gas->MMEX+1);j++){
    //         for(int k=0;k<(gas->MSP+1);k++){
    //             for(int l=0;l<(gas->MSP+1);l++){
    ...
    for(int i=0;i<24;i++){
        // if(i<3)
        //     cudaMemcpy(gas->ISPR[i], gas->d_ISPR+i*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        if(i<3)
            cudaMemcpy(gas->NSLEV[i], gas->d_NSLEV+i*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        // if(i<4)
        //     cudaMemcpy(gas->SPR[i], gas->d_SPR+i*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        // if(i<4)
        //     cudaMemcpy(geom->ICCELL[i], geom->d_ICCELL+i*(geom->NCCELLS+1), (geom->NCCELLS+1)*sizeof(int), cudaMemcpyDeviceToHost);
        if(i<4)
            cudaMemcpy(molecs->PV[i], molecs->d_PV+i*(molecs->MNM+1), (molecs->MNM+1)*sizeof(double), cudaMemcpyDeviceToHost);
        // if(i<5)
        //     cudaMemcpy(geom->CELL[i], geom->d_CELL+i*(geom->NCELLS+1), (geom->NCELLS+1)*sizeof(double), cudaMemcpyDeviceToHost);
        if(i<5)
            cudaMemcpy(gas->TREACL[i], gas->d_TREACL+i*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        if(i<5)
            cudaMemcpy(gas->TREACG[i], gas->d_TREACG+i*(gas->MSP+1),  (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        if(i<6)
            cudaMemcpy(geom->CCELL[i], geom->d_CCELL+i*(geom->NCCELLS+1), (geom->NCCELLS+1)*sizeof(double), cudaMemcpyDeviceToHost);
        // if(i<7)
        //     cudaMemcpy(gas->SP[i], gas->d_SP+i*(gas->MSP+1),  (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
        // if(i<24)
        //     cudaMemcpy(output->VAR[i], output->d_VAR+i*(geom->NCELLS+1), (geom->NCELLS+1)*sizeof(double), cudaMemcpyDeviceToHost);
    }
    // for(int i=0;i<calc->NCLASS+1;i++){
    //     cudaMemcpy(molecs->PX[i], molecs->d_PX+i*(molecs->MNM+1), (molecs->MNM+1)*sizeof(double), cudaMemcpyDeviceToHost);
    // }
    for(int i=0;i<(gas->MSP+1);i++){
        cudaMemcpy(calc->TCOL[i], calc->d_TCOL+i*(gas->MSP+1), (gas->MSP+1)*sizeof(double), cudaMemcpyDeviceToHost);
        cudaMemcpy(gas->ISPRC[i], gas->d_ISPRC+i*(gas->MSP+1),  (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        cudaMemcpy(gas->ISPRK[i], gas->d_ISPRK+i*(gas->MSP+1),  (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
        cudaMemcpy(gas->NSPEX[i], gas->d_NSPEX+i*(gas->MSP+1), (gas->MSP+1)*sizeof(int), cudaMemcpyDeviceToHost);
    }


    //std::cout<<"printf: "<< duration <<'\n';
    
    for(int i=1;i<=geom->NCCELLS;i++){
        calc->TOTCOL=calc->TOTCOL+calc->COLL_TOTCOL[i];
    }
    for(int i=1;i<=molecs->NM;i++)
    {
        if(molecs->IPCELL[i]<0)
            REMOVE_MOL(i); 
    }
    cudaFree(devStates);
    cudaFree(geom->d_ICCELL);
    cudaFree(geom->d_CCELL);
    cudaFree(geom->d_CELL);
    cudaFree(molecs->d_PX);
    cudaFree(molecs->d_PV);
    cudaFree(gas->d_SP);
    cudaFree(gas->d_ISPR);
    if(gas->MMVM>0){
        cudaFree(molecs->d_IPVIB);
        cudaFree(gas->d_SPVM);
    }
    cudaFree(output->d_VAR);
    cudaFree(gas->d_SPM);
    cudaFree(gas->d_SPR);
    cudaFree(calc->d_TCOL);
    cudaFree(gas->d_ISPRC);
    cudaFree(gas->d_SPRC);
    cudaFree(gas->d_TREACL);
    cudaFree(gas->d_TREACG);
    cudaFree(gas->d_SPRT);
    cudaFree(gas->d_SPRP);
    cudaFree(gas->d_ISPRK);
    cudaFree(gas->d_NSPEX);
    cudaFree(gas->d_ISPEX);
    cudaFree(gas->d_SPEX);
    cudaFree(gas->d_NEX);
    // cudaFree(gas->d_SPREX);
    cudaFree(gas->d_NSLEV);
    cudaFree(gas->d_QELC);
    colltime=duration;
    return;
} 
